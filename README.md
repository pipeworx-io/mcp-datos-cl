...
`tools/list` at `https://gateway.pipeworx.io/datos-cl/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/datos_cl_package_list \
  -H 'Content-Type: application/json' \
  -d '{"limit":10,"offset":0}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/datos_cl_package_list`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
